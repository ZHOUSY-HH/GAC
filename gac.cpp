...
void chart::changeC(int i11, int j11, int label1)	//改变上下左右的值,label为0执行more，label为1执行less，实际上不是1就执行more了。
{
	int i1, j1, label;
	queue<int> my_queue;
	my_queue.push(i11);
	my_queue.push(j11);
	my_queue.push(label1);	//初始化队列;
	bool change;
	while (my_queue.size())
	{
		change = false;
		i1 = my_queue.front();	//把队列的三个值pop出来
		my_queue.pop();
		j1 = my_queue.front();
		my_queue.pop();
		label = my_queue.front();
		my_queue.pop();
		...
		if (change)	//如果值域发生了改变，那么相关的约束就要添加进去，这里暂时不做优化。
		{
			for (int tempi = 0; tempi < data[i1][j1].less.size(); tempi++)
			{
				my_queue.push(data[i1][j1].less[tempi].first);
				my_queue.push(data[i1][j1].less[tempi].second);
				my_queue.push(0);
			}
			for (int tempi = 0; tempi < data[i1][j1].more.size(); tempi++)
			{
				my_queue.push(data[i1][j1].more[tempi].first);
				my_queue.push(data[i1][j1].more[tempi].second);
				my_queue.push(1);
			}
		}
	}
